...
export class AmericanasResultadoPesquisaPage extends SimplePage {
    ...
    labelNumeroProduto = element(
        by.xpath(`//div[@class='form-group display-sm-inline-block']/span`)
    );

    botaoNext = element(by.xpath(`//li//span[@aria-label='Next']`));

    botaoNextDesativado = element(
        by.xpath(`//li[@class='disabled']//span[@aria-label='Next']`)
    );
    ...
    async validarResultadoPesquisa() {
        ...
        if (await this.paginacaoEstaVisivel()) {
            ...
            Utils.assertEquals(
                elementosEncontradosNaPaginaDaTabela.toString(),
    ...
    async pegarNumeroTotalDeProdutosEncontrados() {
        ...
        let numeroProdutos = await Utils.pegarAPrimeiraPalavraDaString(
            numeroConstandoLabel
        );
    ...
    async pegarNumeroDeProdutosNaTabela() {
        var produtosNaPaginaDaTabela = element.all(
            by.xpath(`//div[@data-tracker='productgrid-main']//h2`)
        );
    ...
    async compararNomeProdutosComPesquisa(pesquisa: string) {
        var produtosNaPaginaDaTabela = await element.all(
            by.xpath(`//div[@data-tracker='productgrid-main']//h2`)
        );
    ...
    async selecionarProdutoAleatorio() {
        ...
        var precosNaPaginaDoResultado = await element.all(
            by.xpath(
                `//div[@data-tracker='productgrid-main']//div[@class='PriceWrapper-bwhjk3-13 IjiIU ViewUI-sc-1ijittn-6 iXIDWU']/span`
            )
        );

        let index = Utils.gerarnumeroaleatorio(0, produtosNaPaginaDaTabela.length);

        let nomeProduto: string = await this.pegarTexto(
            produtosNaPaginaDaTabela[index]
        );

        let precoProduto: string = await this.pegarTexto(
            precosNaPaginaDoResultado[index]
        );
