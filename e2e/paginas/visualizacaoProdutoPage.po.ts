...
export class VisualizacaoProdutoPagePo extends SimplePage {
    ...
    labelPreco = element(by.xpath(`//span[@class='price__SalesPrice-ej7lo8-2 kjGSBk TextUI-sc-12tokcy-0 bLZSPZ']`));

    botaoSimContinuarCompra = element(by.xpath(`//span[contains(text(),'Sim, continuar')]`));

    async validarNomeProduto(produto: string) {

        Utils.assertEquals(produto, Utils.paraMinusculo(await this.pegarTexto(this.nomeProduto)));
    }
